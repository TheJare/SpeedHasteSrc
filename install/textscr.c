...
PUBLIC bool TXS_InputString(int x1, int x2, int y,
    ...
    back = TXS_SaveBox(x1, y, x2+2, y+3+1);
    TXS_DrawShadedBox(x1,   y,   x2,   y+3, TXS_DoubleFrame, CDLGF, CDLGI);
    TXS_FillBox(x1+1,   y+1,   x2-1,   y+3-1,  ' ', CDLGI);
    TXS_DrawString(x1+1, x2-1, y+1, caption, CDLGTIT, TXS_STCENTER);
    strncpy(buf, s, sizeof(buf));
    buf[sizeof(buf)-1] = '\0';
    l = strlen(buf);
    while (!leave) {
        byte c;
        TXS_DrawString(x1+2, x1+2+l-1, y+2, buf, col, TXS_STRJUST);
        TXS_FillBox(x1+2+l, y+2, x2-2, y+2, '°', col);
